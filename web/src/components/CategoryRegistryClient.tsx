...
import { Search, ListTree, Filter, ArrowLeft, Copy, Activity, ShieldAlert, FileText, Database } from "lucide-react";
import { useScrollRestoration } from "@/hooks/useScrollRestoration";

export default function CategoryRegistryClient({ data }: { data: any }) {
  useScrollRestoration("registry-category");
  ...
  const filteredEntries = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter(e => 
        ...
        (e.category || '').toLowerCase().includes(q) // Use e.title which we mapped to 'title' in parser
      );
    }

    if (domainFilter !== "all") {
      result = result.filter(e => (e.domain || '').toLowerCase() === domainFilter.toLowerCase());
    }
    ...
    result.sort((a, b) => {
      ...
      if (sortOption === 'prefix-asc') return (a.prefix || '').localeCompare(b.prefix || '');
      if (sortOption === 'alphabetical') return (a.title || '').localeCompare(b.title || '');
      return 0;
    });

    return result;
  }, [data.entries, searchQuery, domainFilter, statusFilter, sortOption]);
  ...
  const copyPrefix = (prefix: string, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12 w-full">
      <nav className="flex items-center text-sm text-muted mb-8 pb-2">
        ...
        <span className="text-foreground">Category Registry</span>
      </nav>

      <div className="mb-12">
        <h1 className="text-4xl font-semibold mb-4 flex items-center">
          <ListTree className="mr-4 text-amber-500" size={36} />
          Category Registry
        </h1>
        {data.intro.length > 0 && (
          <div className="prose prose-invert prose-amber max-w-3xl text-muted text-lg leading-relaxed">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
        ...
        <div className="bg-amber-500/5 border border-amber-500/20 p-5 rounded-xl flex items-center justify-between">
           <div>
             <div className="text-xs text-amber-500/70 uppercase tracking-wider mb-2">Total Categories</div>
             <div className="font-mono text-3xl text-amber-500">{data.entries.length}</div>
           </div>
           <ListTree size={48} className="text-amber-500/20" />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
        <section>
          <h2 className="text-xl font-semibold mb-6 flex items-center">
            <Database className="mr-2 text-amber-500" size={20} /> Registry Fields
          </h2>
      ...
      <section className="mb-16">
        <h2 className="text-xl font-semibold mb-6 flex items-center">
          <ShieldAlert className="mr-2 text-rose-500" size={20} /> Registry Rules & Governance
        </h2>
        <div className="bg-rose-500/5 border border-rose-500/20 rounded-xl p-6 md:p-8">
          <ul className="space-y-4">
            {data.rules.map((rule: string, i: number) => (
              <li key={i} className="flex items-start text-rose-500/90 text-sm md:text-base">
                <span className="font-mono bg-rose-500/10 text-rose-500 rounded px-2 py-0.5 mr-4 text-xs mt-0.5">{i + 1}</span>
      ...
      <section className="mb-12">
        <h2 className="text-xl font-semibold mb-6 flex items-center">
          <FileText className="mr-2 text-amber-500" size={20} /> Category Registry Entries
        </h2>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 bg-card border border-border p-4 rounded-xl">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              type="text"
              placeholder="Search ID, Prefix, or Category..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-background border border-border rounded-lg pl-10 pr-4 py-2 text-sm focus:outline-none focus:border-amber-500 transition-colors"
            />
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <select
              ...
              className="bg-background border border-border rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-amber-500"
            >
              <option value="id-asc">Category ID</option>
              <option value="prefix-asc">Prefix</option>
              <option value="alphabetical">Category Name</option>
            </select>
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl overflow-x-auto max-h-[600px] overflow-y-auto">
          <table className="w-full text-left text-sm relative">
            <thead className="bg-background/90 backdrop-blur-sm border-b border-border text-muted sticky top-0 z-10">
              <tr>
                <th className="px-6 py-4 font-medium uppercase tracking-wider text-xs">Category ID</th>
                <th className="px-6 py-4 font-medium uppercase tracking-wider text-xs">Prefix</th>
                <th className="px-6 py-4 font-medium uppercase tracking-wider text-xs w-1/3">Category Name</th>
            ...
            <tbody className="divide-y divide-border">
              {filteredEntries.map((e, i) => (
                <tr key={i} className="hover:bg-background/50 transition-colors group">
                  <td className="px-6 py-4 font-mono font-medium">{e.id}</td>
                  <td className="px-6 py-4 font-mono font-medium">
                    <div className="flex items-center gap-2">
                      <Link href={`/atlas/${(e.prefix || '').toLowerCase()}`} className="text-amber-500 hover:underline">{e.prefix}</Link>
                      <button onClick={(ev) => copyPrefix(e.prefix, ev)} className="text-muted hover:text-foreground opacity-0 group-hover:opacity-100 transition-opacity">
                        <Copy size={12} />
                      </button>
                    </div>
                  </td>
                  <td className="px-6 py-4 font-medium text-foreground">{e.title}</td>
                  <td className="px-6 py-4 text-muted">{e.domain || '-'}</td>
